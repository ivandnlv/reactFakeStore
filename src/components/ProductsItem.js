...
const ProductsItem = ({ image, id, title, price, description, category, loading }) => {
  const [descr, setDescr] = useState(descriptionNormalize());
  ...
  return (
    <div className="p-8 rounded-lg bg-slate-400 flex flex-col items-center  show">
      <p className="bg-slate-200 rounded-full p-2 text-slate-800 font-bold inline-block m-auto">
        {category}
      </p>
      <img src={image} alt={'product' + id} className="w-40 h-40 mt-3 p-2 bg-white rounded-lg" />
